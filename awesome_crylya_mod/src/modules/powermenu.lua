...
return function(s)
  ...
  local profile_picture = wibox.widget {
    image = icondir .. "profile_pic.svg",
    -- image = gears.color.recolor_image(icondir .. "vincent_law.svg", color["faded_green"]),
    resize = true,
            ...
            {
              {
                ...
                image = gears.color.recolor_image(icon, color["light0_soft"]),
                -- image = icon,
                resize = true,
  ...
  local shutdown_button = button("Shutdown", icondir .. "shutdown.svg", color["dark1"], shutdown_command)
  local reboot_button = button("Reboot", icondir .. "reboot.svg", color["dark1"], reboot_command)
  local suspend_button = button("Suspend", icondir .. "suspend.svg", color["dark1"], suspend_command)
  local logout_button = button("Logout", icondir .. "logout.svg", color["dark1"], logout_command)
  local lock_button = button("Lock", icondir .. "lock.svg", color["dark1"], lock_command)

  -- Signals to change color on hover
  Hover_signal(shutdown_button.background, color["dark2"], color["light0"])
  Hover_signal(reboot_button.background, color["dark2"], color["light0"])
  Hover_signal(suspend_button.background, color["dark2"], color["light0"])
  Hover_signal(logout_button.background, color["dark2"], color["light0"])
  Hover_signal(lock_button.background, color["dark2"], color["light0"])
