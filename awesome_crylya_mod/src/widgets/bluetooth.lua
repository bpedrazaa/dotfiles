...
return function()
    ...
    {
      {
        {
          id = "icon",
          image = gears.color.recolor_image(icondir .. "bluetooth-off.svg", color["light0_soft"]),
  ...
  awful.widget.watch(
    ...
    function(_, stdout)
      ...
      bluetooth_widget.icon_margin.icon_layout.icon:set_image(gears.color.recolor_image(icon .. ".svg", color["light0_soft"]))
    end,
    bluetooth_widget
  )

  -- Signals
  Hover_signal(bluetooth_widget, color["dark2"], color["light0"])
