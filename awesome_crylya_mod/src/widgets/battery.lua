...
return function()
        ...
        {
          {
            {
              id = "icon",
              image = gears.color.recolor_image(icondir .. "battery-unknown.svg", color["light0_soft"]),
      ...
      function(stdout)
        ...
        if status == 'fully-charged' or status == 'charging' and battery_percentage == 100 then
          ...
          battery_widget.container.battery_layout.icon_margin.icon_layout.icon:set_image(gears.surface.load_uncached(
            gears.color.recolor_image(icondir .. icon .. '.svg', color["light0_soft"])))
          return
        end
  ...
  Hover_signal(battery_widget, color["dark2"], color["light0"])
  ...
  watch(
    ...
    function(widget, stdout)
      ...
      if status == nil or status == '' then
        ...
        battery_tooltip:set_text('No battery found')
        battery_widget.container.battery_layout.icon_margin.icon_layout.icon:set_image(gears.surface.load_uncached(
          gears.color.recolor_image(icondir .. 'battery-off' .. '.svg', color["light0_soft"])))
      end
