...
return function(widget)
          ...
          {
            {
              ...
              image = gears.color.recolor_image(icon_dir .. "gpu.svg", color["light0_soft"]),
              resize = false
            },
            id = "icon_layout",
  ...
  Hover_signal(gpu_usage_widget, color["dark2"], color["light0"])
          ...
          {
            {
              id = "icon",
              widget = wibox.widget.imagebox,
              image = gears.color.recolor_image(icon_dir .. "cpu.svg", color["light0_soft"]),
              resize = false
            },
            id = "icon_layout",
            widget = wibox.container.place
  ...
  watch(
    ...
    function(_, stdout)
      ...
      if temp_num then
        ...
        if temp_num < 50 then
          temp_color = color["light0_soft"]
          temp_icon = gears.color.recolor_image(icon_dir .. "thermometer-low.svg", color["light0_soft"])
        elseif temp_num >= 50 and temp_num < 80 then
          temp_color = color["light0_soft"]
          temp_icon = gears.color.recolor_image(icon_dir .. "thermometer.svg", color["light0_soft"])
        elseif temp_num >= 80 then
          temp_color = color["light0_soft"]
          temp_icon = gears.color.recolor_image(icon_dir .. "thermometer-high.svg", color["light0_soft"])
        end
      else
        temp_num = "NaN"
        temp_color = color["light0_soft"]
      ...
      Hover_signal(gpu_temp_widget, color["dark2"], color["light0"])
      gpu_temp_widget.container.gpu_layout.icon_margin.icon_layout.icon:set_image(temp_icon)
      gpu_temp_widget:set_bg(color["dark1"])
